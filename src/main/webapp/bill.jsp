...
try{
	...
	ResultSet rs=st.executeQuery("select sum(total) from cart where orderid='"+orderid+"'");
	while(rs.next()){
		total=rs.getInt(1);
	}
	ResultSet rs2=st.executeQuery("select * from users a inner join cart b on a.email = b.email where orderid='"+orderid+"'");
...
<hr>
<div class="left-div"><h3>Thankyou for shopping!</h3></div>
<div class="right-div-right"><h3>Name:<%=rs2.getString(1) %></h3></div>
...
<%
ResultSet rs1=st.executeQuery("select*from cart inner join product where cart.product_id=product.id and cart.orderid='"+orderid+"'");
while(rs1.next()){
	sno=sno+1;
